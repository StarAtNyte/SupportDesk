...
impl Connection {
    ...
    /// Returns true if the given peer ID is in the trusted-peers config option.
    /// Trusted peers are auto-accepted without password or click approval.
    /// Set via: Config::set_option("trusted-peers", "id1,id2,id3")
    fn is_trusted_peer(peer_id: &str) -> bool {
        let trusted = Config::get_option("trusted-peers");
        if trusted.is_empty() {
            return false;
        }
        trusted.split(',').any(|id| id.trim() == peer_id)
    }
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
                ...
                && lr.username != Config::get_id()
            {
                self.send_login_error(crate::client::LOGIN_MSG_OFFLINE)
                    .await;
                return false;
            } else if Self::is_trusted_peer(&lr.my_id) {
                // Auto-accept connections from trusted agent IDs (no password or click needed)
                self.authorized = true;
                #[cfg(target_os = "linux")]
